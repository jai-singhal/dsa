// CSC 348: Introduction to Compiler Design: 2018 Spring
// Assignment #1
// Last Modified 2018 March 28
// Purpose:
// To go over:
// Recursive descent parsers
// Overview:
// Finish my C++ program that implements a recursive-descent parser of a simple language of cleaning one's house in English. The
// language can handle:
// Mop the bathroom.
// Wash the dishes.
...
// etc.
// Assignment:
// 1. Please copy and paste the following:
...
= WASH_KEYWORD_SYM;
keywordToSymbolTMap_["scrub"]
= SCRUB_KEYWORD_SYM;
keywordToSymbolTMap_["vacuum"]
= VACUUM_KEYWORD_SYM;
keywordToSymbolTMap_["sweep"]
= SWEEP_KEYWORD_SYM;
keywordToSymbolTMap_["mop"]
= MOP_KEYWORD_SYM;
keywordToSymbolTMap_["dishes"]
= DISHES_KEYWORD_SYM;
keywordToSymbolTMap_["pots"]
= POTS_KEYWORD_SYM;
keywordToSymbolTMap_["windows"]
= WINDOWS_KEYWORD_SYM;
keywordToSymbolTMap_["sink"]
= SINK_KEYWORD_SYM;
keywordToSymbolTMap_["toilet"]
= TOILET_KEYWORD_SYM;
keywordToSymbolTMap_["shower"]
= SHOWER_KEYWORD_SYM;
keywordToSymbolTMap_["hall"]
= HALL_KEYWORD_SYM;
keywordToSymbolTMap_["foyer"]
= FOYER_KEYWORD_SYM;
keywordToSymbolTMap_["bedroom"]
= BEDROOM_KEYWORD_SYM;
keywordToSymbolTMap_["bathroom"]
= BATHROOM_KEYWORD_SYM;
keywordToSymbolTMap_["kitchen"]
= KITCHEN_KEYWORD_SYM;
advance();
}
// PURPOSE: To release the resources of '*this'. No parameters. No return
...
3. Please finish parseRoomVerb():
It should recognize (and then delete()) the tokens for VACUUM_KEYWORD_SYM, SWEEP_KEYWORD_SYM and MOP_KEYWORD_SYM.
If you detect an error then simply throw a string that describes the problem. In my code I had the following:
throw "Expected \"vacuum\", \"sweep\", \"mop\" or end";
4. Please finish parseWashItem():
It should recognize (and then delete()) the tokens for DISHES_KEYWORD_SYM, POTS_KEYWORD_SYM, WINDOWS_KEYWORD_SYM,
SINK_KEYWORD_SYM, TOILET_KEYWORD_SYM, and SHOWER_KEYWORD_SYM.
If you detect an error then simply throw a string that describes the problem. In my code I had the following:
throw "Expected \"dishes\", \"pots\", \"windows\","
" \"sink\", \"toilet\" or \"shower\"";
5. Please finish parseWashList():
It should recognize grammar rules 8 and 9 above.
(8) WL -> WI WL
(9) WL ->
6. Please finish parseRoomItem():
It should recognize (and then delete()) the tokens for HALL_KEYWORD_SYM, BEDROOM_KEYWORD_SYM, BATHROOM_KEYWORD_SYM,
KITCHEN_KEYWORD_SYM, and FOYER_KEYWORD_SYM.
If you detect an error then simply throw a string that describes the problem. In my code I had the following:
throw "Expected \"hall\", \"foyer\", \"bedroom\" \"bathroom\" or \"kitchen\"";
7. Please finish parseRoomList():
It should recognize grammar rules 10 and 11 above.
(10) RL -> RI RL
(11) RL ->
8. Please finish parseWashSentence():
It should recognize grammar rule 4 above. WS -> WV the WL period
If you detect an error then simply throw a string that describes the problem. In my code I had the following:
throw "Expected \"the\"";
throw "Expected \".\"";
throw "Expected \"wash\" or \"scrub\"";
9. Please finish parseRoomSentence():
It should recognize grammar rule 5 above. RS -> RV the RL period
If you detect an error then simply throw a string that describes the problem. In my code I had the following:
throw "Expected \"the\"";
throw "Expected \".\"";
throw "Expected \"wash\" or \"scrub\"";
10. Please finish parseS():
(Done for you.)
11. Hints:
a. If the same non-terminal is on the right-hand-side, then that means a recursive call to the same function.
The methods you have to help you are:
Method: Function:
tokenizer.peek()
It returns the symbol_ty value for the next symbol on the input stream, without changing the input
stream. When you reach the end of input it returns the value END_OF_INPUT_SYM.
tokenizer.advance()
It returns a pointer to the instance of Symbol that was next in the token stream, and internally goes to the
next symbol. Once you are thru with them, all Symbol instances should be delete()d.
...
This house looks better already!
$ ./cleaningLang
Expression to compute: Wash the dishes pots.
This house looks better already!
$ ./cleaningLang
Expression to compute: wash the
Expected "."
$ ./cleaningLang
Expression to compute: wash
Expected "the"
$ ./cleaningLang
Expression to compute: mop.
Expected "the"
$ ./cleaningLang
Expression to compute: Mop the kitchen
Expected "."